...
function storyDateContext(data = {}) {
  const eventDate = parseStoryDate(
    data.specialDate || data.publishedAt || data.createdAt
  );
  ...
  const daysAgo = Math.round((currentDate - eventDate) / 86400000);

  if (daysAgo <= 0) {
    return { prefix: "Hoje", lowerPrefix: "hoje", daysAgo: 0 };
  }

  if (daysAgo === 1) {
    return { prefix: "Ontem", lowerPrefix: "ontem", daysAgo };
  }

  if (daysAgo <= 6) {
    const weekday = eventDate.toLocaleDateString("pt-BR", { weekday: "long" });
    const prefix = `Na última ${weekday}`;
    return { prefix, lowerPrefix: prefix.toLocaleLowerCase("pt-BR"), daysAgo };
  }
  ...
  const prefix = `No dia ${formattedDate}`;
...
export function buildPetStory(data = {}) {
  ...
  const dateContext = storyDateContext(data);
  ...
  if (type.id === "SURGERY") {
    ...
    base.intro = [
      `${dateContext.prefix} foi um dia de coragem.`,
  ...
  if (type.id === "DISCHARGE") {
    ...
    base.intro = [
      `${dateContext.prefix} chegou o momento de voltar para casa.`,
  ...
  if (type.id === "ADOPTION") {
    ...
    const momentText = {
      FIRST_DAY: `${dateContext.prefix} começou o primeiro capítulo dessa nova vida em família.`,
      ANNIVERSARY: `${dateContext.prefix} celebramos o dia em que essa família ficou completa.`,
      NEW_HOME: `${dateContext.prefix}, um novo lar ganhou mais amor, alegria e companhia.`,
    }[moment] || `${dateContext.prefix} começou uma história para a vida inteira.`;
  ...
  if (type.id === "BIRTHDAY") {
    ...
    base.intro = [
      `${dateContext.prefix}, o dia amanheceu mais alegre.`,
      `É dia de celebrar ${age} de uma história muito especial.`,
      `${dateContext.prefix}, a festa é de ${petName}!`,
    ];
    base.emotionTitle = "Mais um ano espalhando alegria, carinho e amor por onde passa.";
    base.chapters = [
      ["Um dia de festa", `${dateContext.prefix} celebramos ${age} de ${petName}.`],
    ...
    base.defaultMessage = `${dateContext.prefix} celebramos ${age} de uma vida que torna todos os dias mais felizes.\n\n${petName} é motivo de sorrisos, carinho e muitas histórias inesquecíveis.\n\nQue nunca faltem ${favorite}, saúde e amor.\n\nFeliz aniversário e muitos capítulos lindos pela frente!`;
  }

  if (type.id === "VACCINATION") {
    const moment = {
      FIRST: `${dateContext.prefix} foi dado o primeiro passo para uma vida protegida.`,
      BOOSTER: `${dateContext.prefix}, a proteção ficou ainda mais forte.`,
      ANNUAL: "Mais um ano de cuidado e prevenção em dia.",
    }[answer(answers, "vaccineMoment")] || `${dateContext.prefix} foi dado mais um passo para uma vida protegida.`;
