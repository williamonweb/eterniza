...
function TributeCard({ tribute, onPublish, onCopy, onShare, onDelete, highlighted }) {
  ...
  const fullPublicUrl =
    publicUrl && typeof window !== "undefined"
      ...
      : publicUrl;
  const hasQrCode = ["premium", "eterno"].includes(String(planSlug).toLowerCase());
  const qrCodeUrl = fullPublicUrl
    ? `https://api.qrserver.com/v1/create-qr-code/?size=1000x1000&format=png&margin=24&data=${encodeURIComponent(fullPublicUrl)}`
    : "";
      ...
      <div className="tribute-content">
        ...
        {isPublished && publicUrl ? (
          <>
            ...
            {hasQrCode && qrCodeUrl && (
              <a className="qr-action" href={qrCodeUrl} target="_blank" rel="noreferrer">
                ▦ Baixar QR Code
              </a>
            )}
