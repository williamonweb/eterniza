  ...
  try {
    ...
    const [
      ...
      prisma.tribute.findMany({
        select: {
          ...
          createdAt: true,
          updatedAt: true,
          user: { select: { name: true, email: true, notes: true } },
    ...
    return NextResponse.json({
      ...
      tributes: tributes.map((tribute) => ({
        ...
        userEmail: tribute.user?.email?.endsWith('@guest.eternizas.invalid') ? tribute.user?.notes || "Visitante sem pagamento" : tribute.user?.email || "",
