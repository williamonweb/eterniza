...
function photoSource(photo) {
  ...
  return photo.dataUrl || photo.dataURL || photo.url || photo.src || "";
}
      ...
      <section ref={storyRef} className={`story ${storyVisible?"visible":""}`}>
        <header className="story-hero">
          ...
          <div
            ...
            style={photos[0]?{"--photo-bg":`url(${photoSource(photos[0])})`}:undefined}
          >
            {photos[0]?<img src={photoSource(photos[0])} alt={data.petName}/>:<div className="photo-placeholder">🐾</div>}
            <i/>
            <b>{data.petName}</b>
          </div>
          ...
          <section className="photo-chapters">
            {photos.slice(1).map((photo,index)=>(
              <article className={`photo-chapter ${index%2?"reverse":""}`} key={`${photo.name||"foto"}-${index}`}>
                <div
                  className="chapter-photo-card"
                  style={{"--photo-bg":`url(${photoSource(photo)})`}}
                >
                  <img src={photoSource(photo)} alt={`${data.petName} ${index+2}`}/>
