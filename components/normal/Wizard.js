...
export default function Wizard({ step }) {
  ...
  async function goNext(){
    ...
    if(step==='musica'&&data.youtubeLink.trim()&&!youtubeId(data.youtubeLink))return setError('Cole um link válido do YouTube ou deixe o campo em branco.');
  ...
  return <Shell compact><div className="normal-kicker">Criação simples, em poucos passos</div><div className="normal-progress" aria-label="Progresso da criação">{steps.map((name,i)=><span key={name} className={i===index?'active':''}>{i+1}. {stepNames[i]}</span>)}</div><div className="normal-card">
    ...
    {step==='musica'&&<><h1 className="normal-heading">Qual música lembra esse momento?</h1><p className="normal-subtitle">Cole o link de um vídeo do YouTube. Quem abrir a página poderá tocar a música.</p><label className="normal-field">Link da música no YouTube<input type="url" value={data.youtubeLink} onChange={e=>update({youtubeLink:e.target.value})} placeholder="https://www.youtube.com/watch?v=..."/><small>Opcional. Funciona com links do YouTube e do YouTube Shorts.</small></label>{youtubeId(data.youtubeLink)&&<p className="normal-hint">✓ Música pronta para aparecer na sua página.</p>}</>}
