...
const empty = { builderVersion:'2.0', recipient:null, title:'', receiverName:'', senderName:'', subtitle:'', message:'', specialDate:'', birthDate:'', deathDate:'', memorialType:'memoria', categoryDrafts:{}, photos:[], youtubeLink:'', plan:null };
const categoryFields=['title','receiverName','senderName','subtitle','message','specialDate','birthDate','deathDate','memorialType'];
...
export default function Wizard({ step }) {
  ...
  function chooseCategory(id) {
    setData(current=>{
      const previous=current.recipient?.id;
      if(previous===id)return current;
      const drafts={...current.categoryDrafts};
      if(previous)drafts[previous]=Object.fromEntries(categoryFields.map(field=>[field,current[field] ?? empty[field]]));
      const next={...current,...Object.fromEntries(categoryFields.map(field=>[field,empty[field]])),...drafts[id],recipient:{id},categoryDrafts:drafts};
      latest.current=next;
      return next;
    });
    setError('');
  ...
  async function goNext(){
    ...
    if(step==='datas'&&selected.id==='homenagem'&&data.memorialType!=='vida'&&data.birthDate&&data.deathDate&&data.deathDate<data.birthDate)return setError('A data de falecimento deve ser igual ou posterior à data de nascimento.');
  ...
  return <Shell compact storyWide={step==='preview'}><div className="normal-kicker">Criação simples, em poucos passos</div><div className="normal-progress" aria-label="Progresso da criação">{steps.map((name,i)=><span key={name} className={i===index?'active':''}>{i+1}. {stepNames[i]}</span>)}</div><div className="normal-card">
    ...
    {step==='tipo'&&<><div className="normal-center"><h1 className="normal-heading">Escolha o tipo de página</h1><p className="normal-subtitle">Qual momento você quer celebrar ou recordar?</p></div><div className="normal-grid normal-type-grid">{categories.map(cat=><button type="button" key={cat.id} aria-pressed={data.recipient?.id===cat.id} className={`normal-choice ${data.recipient?.id===cat.id?'selected':''}`} onClick={()=>chooseCategory(cat.id)}><img src={cat.image} alt=""/><strong><span className="normal-choice-icon">{cat.icon}</span>{cat.label}</strong></button>)}</div><p className="normal-type-note">Toda história merece ser lembrada. ♡</p></>}
    {step==='historia'&&<><span className="normal-pill">{selected.icon} {selected.label}</span><h1 className="normal-heading">{selected.id==='homenagem'?'Conte a história de quem você quer homenagear':'Conte essa história do seu jeito'}</h1><p className="normal-subtitle">{selected.id==='homenagem'?'Compartilhe lembranças, palavras de carinho e o que torna essa pessoa especial.':'Escreva como se estivesse conversando com quem vai receber a página.'}</p>
      {selected.id==='homenagem'&&<fieldset className="normal-memorial-choice"><legend>Que tipo de homenagem você quer criar?</legend><div>{[['memoria','Em memória de alguém'],['vida','Para alguém em vida']].map(([value,label])=><button type="button" key={value} aria-pressed={(data.memorialType||'memoria')===value} className={(data.memorialType||'memoria')===value?'selected':''} onClick={()=>update({memorialType:value})}>{label}</button>)}</div></fieldset>}
      <label className="normal-field">Título da página<input required maxLength={90} value={data.title} onChange={e=>update({title:e.target.value})} placeholder={selected.id==='homenagem'?'Ex.: Em memória de Antônio Silva':'Ex.: Nossos 5 anos de amor'} /></label><div className="normal-two"><label className="normal-field">{selected.id==='homenagem'?'Nome da pessoa homenageada':'Para quem é esta página?'}<input maxLength={90} value={data.receiverName} onChange={e=>update({receiverName:e.target.value})} placeholder={selected.id==='homenagem'?'Ex.: Antônio Silva':'Nome ou apelido'} /></label><label className="normal-field">{selected.id==='homenagem'?'Quem está criando esta homenagem? (opcional)':'Seu nome (opcional)'}<input maxLength={90} value={data.senderName} onChange={e=>update({senderName:e.target.value})} placeholder="Como quer assinar?" /></label></div><label className="normal-field">{selected.id==='homenagem'?'Uma frase para lembrar essa pessoa (opcional)':'Uma frase de abertura (opcional)'}<input maxLength={140} value={data.subtitle} onChange={e=>update({subtitle:e.target.value})} placeholder={selected.id==='homenagem'?'Ex.: O amor permanece em cada lembrança':'Ex.: Cada dia ao seu lado é especial'} /></label><label className="normal-field">{selected.id==='homenagem'?'Conte uma lembrança ou escreva sua homenagem':'Sua mensagem'}<textarea maxLength={2000} value={data.message} onChange={e=>update({message:e.target.value})} placeholder={selected.id==='homenagem'?'Fale sobre quem essa pessoa é ou foi, os momentos compartilhados e o que você gostaria de guardar...':'Conte uma lembrança, um agradecimento ou o que sente...'}/><small>{data.message.length}/2000 caracteres</small></label></>}
    {step==='datas'&&<><h1 className="normal-heading">{selected.id==='homenagem'?'Datas para lembrar':'Uma data para guardar'}</h1><p className="normal-subtitle">{selected.id==='homenagem'?(data.memorialType==='vida'?'Se quiser, informe uma data especial na vida dessa pessoa.':'Se quiser, informe as datas de nascimento e falecimento para aparecerem junto ao retrato. Ambas são opcionais.'):'Esta data ajuda a mostrar o tempo de história e a contagem até o próximo aniversário.'}</p>{selected.id==='homenagem'&&data.memorialType!=='vida'?<div className="normal-two"><label className="normal-field">Nascimento<input type="date" value={data.birthDate||''} onChange={e=>update({birthDate:e.target.value})}/></label><label className="normal-field">Falecimento<input type="date" value={data.deathDate||''} onChange={e=>update({deathDate:e.target.value})}/></label></div>:<label className="normal-field">{selected.id==='namoro'?'Quando a história de vocês começou?':selected.id==='casamento'?'Qual foi a data do casamento?':selected.id==='bebe'?'Quando o bebê nasceu?':selected.id==='homenagem'?'Qual data você gostaria de lembrar?':selected.id==='pet'?'Quando seu pet chegou ou nasceu?':'Qual é a data especial?'}<input type="date" value={data.specialDate||''} onChange={e=>update({specialDate:e.target.value})}/><small>Você pode deixar em branco e seguir.</small></label>}{selected.id!=='homenagem'&&<div className="normal-card" style={{background:'#f7f0e6'}}><strong>O que aparecerá na página?</strong><p style={{marginBottom:0}}>Dias de história, anos e contagem até o próximo aniversário, atualizados automaticamente.</p></div>}</>}
    {step==='fotos'&&<><h1 className="normal-heading">Escolha suas fotos favoritas</h1><p className="normal-subtitle">{selected.id==='homenagem'?'A primeira foto aparecerá como retrato da pessoa homenageada. As demais formarão a galeria.':'Uma foto já conta uma história. A primeira será a capa da página.'}</p><label className="normal-field">Adicionar fotos<input type="file" accept="image/*" multiple onChange={addPhotos}/><small>{data.photos.length} de até {maxPhotos} fotos · O plano escolhido pode ter um limite menor.</small></label><div className="normal-photo-grid">{data.photos.map((photo,i)=><div className="normal-photo" key={i}><img src={photo} alt={`Foto ${i+1}`}/><button type="button" aria-label={`Remover foto ${i+1}`} onClick={()=>update({photos:data.photos.filter((_,n)=>n!==i)})}>×</button></div>)}</div></>}
