...
export default function Wizard({ step }) {
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try {
        ...
        if(!auth.ok) {window.location.replace(`/login?redirect=${encodeURIComponent('/criar/'+step)}`);return;}
        const uid=auth.user.id;setUserId(uid);
        ...
        if(draftId){
          ...
          if(res.ok){const saved=await res.json();if(alive){const content={...empty,...saved.tribute.content};latest.current=content;setData(content);idRef.current=draftId;setId(draftId);}}
  ...
  function save(snapshot=latest.current){
    if(!readyRef.current || !userId || !snapshot.recipient?.id) return Promise.resolve(idRef.current);
    setSaving(true);
  ...
  async function goNext(){
    ...
    try {const savedId=await save();router.push(index===steps.length-1 ? `/planos?id=${savedId}` : `/criar/${steps[index+1]}${savedId?`?id=${savedId}`:''}`);}catch(e){setError(e.message);}
  }
  function goBack(){router.push(index===0?'/minhas-paginas':`/criar/${steps[index-1]}${id?`?id=${id}`:''}`);}
  ...
  return <Shell compact><div className="normal-kicker">Criação simples, em poucos passos</div><div className="normal-progress" aria-label="Progresso da criação">{steps.map((name,i)=><span key={name} className={i===index?'active':''}>{i+1}. {stepNames[i]}</span>)}</div><div className="normal-card">
    {!ready ? <p>Carregando sua página…</p> : <>
    {step==='tipo'&&<><h1 className="normal-heading">Qual momento vamos celebrar?</h1><p className="normal-subtitle">Escolha o tema que mais combina com a sua história. Você pode mudar depois.</p><div className="normal-grid">{categories.map(cat=><button type="button" key={cat.id} className={`normal-choice ${data.recipient?.id===cat.id?'selected':''}`} onClick={()=>update({recipient:{id:cat.id}})}><img src={cat.image} alt=""/><strong>{cat.icon} {cat.label}</strong><small>{cat.subtitle}</small></button>)}</div></>}
